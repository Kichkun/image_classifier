...
class SplitFolder():
    def __init__(self, folder, dataset_folder, _split=[0.7,0.15,0.15], class_names={'0':'bad', '1':'bad', '2':'good'}, order = {'id':0, 'class':1, 'frame':2, 'else':None}):
        self.folder = folder
        self.dataset_folder = dataset_folder

        _ids = list(set([_file.split('_')[order['id']] for _file in os.listdir(folder)]))
        ...
        for key,val in self._ids.items():
           for _id in tqdm(val):
               ...
               class_name = images[0].split('_')[order['class']]
